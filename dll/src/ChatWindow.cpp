...
ChatWindow::ChatWindow() : _visible(nullptr), _colors(nullptr)
{
    EventManager::Subscribe(GDCL_EVENT_SET_MAIN_PLAYER,  &ChatWindow::OnSetMainPlayerEvent);
    EventManager::Subscribe(GDCL_EVENT_WORLD_PRE_UNLOAD, &ChatWindow::OnWorldPreUnloadEvent);
}
...
void ChatWindow::OnSetMainPlayerEvent()
{
    spChatWindow->FindMagicAddresses();
    spChatWindow->LoadConfig();
}

void ChatWindow::OnWorldPreUnloadEvent()
{
    spChatWindow->_visible = nullptr;
    spChatWindow->_colors = nullptr;
}
...
bool ChatWindow::SetChatColor(ChatType type, uint32_t color)
{
    if (_colors)
    {
    ...
    if (OpenClipboard(nullptr))
    {
        if (HANDLE data = GetClipboardData(CF_TEXT))
        {
            if (char* charData = (char*)GlobalLock(data))
            {
                ...
                while ((*charData != '\0') && (length < MAX_MESSAGE_SIZE))
                {
...
int64_t ChatWindow::HoldKeyUpdate()
{
    ChatWindow& chatWindow = GetInstance();
...
bool ChatWindow::HandleKeyPress(EngineAPI::Input::KeyButtonEvent& event)
{
    if (_visible)
    {
        ...
        switch (event._key)
        {
            ...
            default:
            {
                if ((IsVisible()) && (output != 0) && (text.size() < MAX_MESSAGE_SIZE))
                {
...
bool ChatWindow::HandleKeyEvent(EngineAPI::Input::KeyButtonEvent& event)
{
    ...
    if (IsInitialized())
    {
        if (event._state == EngineAPI::Input::KEY_STATE_DOWN)
        {
            _holdLock = true;
            _holdEvent = event;
            ThreadManager::DeleteThread("chat_window_hold");
            ThreadManager::CreateThread("chat_window_hold", 50, 500, &ChatWindow::HoldKeyUpdate);
...
void ChatWindow::LoadConfig()
{
    ...
    if (std::filesystem::is_regular_file(configPath))
    {
        ...
        SetChatColor(CHAT_TYPE_TRADE, _tradeColor);
    }
}
