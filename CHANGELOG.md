...
- The launcher now properly checks the start and end dates of available seasons to determine which ones are currently active. If no season is currently active, the launcher now notifies the user and terminates instead of launching the game.
- The launcher now uses file sizes rather than MD5 checksums when checking local files for updates.
- Added basic Lua support in the launcher for the GrimLeague season mod.