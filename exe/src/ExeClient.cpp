...
ExeClient* ExeClient::GetInstance()
{
    static ExeClient instance;
    return &instance;
}

bool WriteByteToPipe(void* pipe, uint8_t value)
{
    DWORD bytesWritten;
    return (WriteFile(pipe, &value, 1, &bytesWritten, NULL) && (bytesWritten == 1));
}

bool WriteInt16ToPipe(void* pipe, wchar_t value)
{
    DWORD bytesWritten;
    uint8_t buffer[2];
...
bool WriteInt32ToPipe(void* pipe, uint32_t value)
{
    DWORD bytesWritten;
    uint8_t buffer[4];
...
bool WriteStringToPipe(void* pipe, const std::string& str)
{
    DWORD bytesWritten;
...
bool WriteWideStringToPipe(void* pipe, const std::wstring& str)
{
    uint32_t length = (uint32_t)str.length();
    if (!WriteInt32ToPipe(pipe, length))
        return false;
...
bool WriteSeasonsToPipe(void* pipe, const std::vector<SeasonInfo>& seasons)
{
